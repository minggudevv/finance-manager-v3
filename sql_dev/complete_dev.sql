...
-- Allow authenticated users to insert their profile on signup
...
-- Added as fix for missing app_settings table
...
-- Allow service role to insert (for initial setup via SQL)
CREATE POLICY "Allow service role insert" ON app_settings
...
CREATE POLICY "Allow update for admins only" ON app_settings
  FOR UPDATE TO authenticated
  USING (true)  -- Allow viewing the settings for all authenticated users
  WITH CHECK (
    EXISTS (
